...
bif_dir = os.path.join(script_dir, '..', '..', 'bayesian-networks', 'bif')
sch_dir = os.path.join(script_dir, 'sch1')
...
def pcnf_encoding(dataset):
    ...
    file_idx = 1
    os.makedirs(os.path.join(sch_dir, dataset), exist_ok=True)
    for nvar in network_variables:
        if network_variables[nvar]['is_leaf']:
            for i in range(network_variables[nvar]['dom_size']):
                with open(os.path.join(sch_dir, dataset, f'{file_idx}.cnf'), 'w') as f:
                    f.write(f'p cnf {variable_index} {len(clauses) + 1}\n')
