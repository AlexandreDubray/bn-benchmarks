import os
import re
import subprocess
import time

script_dir = os.path.dirname(os.path.realpath(__file__))
uai_dir = os.path.join(script_dir, '..', '..', 'bayesian-networks', 'uai')
enc1_dir = os.path.join(script_dir, 'enc1')
enc3_dir = os.path.join(script_dir, 'enc3')
enc4_dir = os.path.join(script_dir, 'enc4')
enc4linp_dir = os.path.join(script_dir, 'enc4linp')

def get_network_data(dataset):
    with open(os.path.join(uai_dir, dataset + '.uai')) as f:
        
        lines = f.readlines()
        assert(lines[0].rstrip() == 'BAYES')
        
        number_network_vars = int(lines[1])

        domain_size = []
        for d_size in [int(x) for x in lines[2].rstrip().split(' ')]:
            domain_size.append(d_size)

        is_leaf = [True for _ in range(number_network_vars)]
        cpt_scope = []
        for cpt_score_def in lines[4:(4+number_network_vars)]:
            for variable in [int(x) for x in cpt_score_def.rstrip().split(' ')[1:-1]]:
                is_leaf[variable] = False
        return {'leaf': is_leaf, 'dom_size': domain_size}

def parse_exe_output():
    output = {}
    with open('tmp.cnf') as f:
        lines = f.readlines()
        s = lines[0].rstrip().split(' ')
        output['nvars'] = int(s[2])
        output['nclauses'] = int(s[3])
        output['clauses'] = lines[1:]
        
    weights = {}
    with open('tmp.weight') as f:
        for line in f.readlines()[:-1]:
            s = line.rstrip().split(' ')
            var = int(s[0])
            weight = float(s[1])
            weights[var] = weight
    output['weights'] = weights
    
    variable_map = {}
    with open('tmp.map') as f:
        for line in f:
            s = line.rstrip().split('=')
            var = int(s[0])
            subsplit = s[1].split('][')
            variable_map[var] = [[int(x) for x in re.findall(r'-?\d+', y)] for y in subsplit]
    output['variable_map'] = variable_map
    return output
    
def make_enc_encoding(dataset, transform_cmd, outdir):
    start_transform = time.time()
    subprocess.run(transform_cmd, stdout=subprocess.DEVNULL)
    end_transform = time.time()
    with open(os.path.join(outdir, 'preprocess_time.csv'), 'a') as f:
        f.write(f'{dataset},{end_transform - start_transform}\n')
    os.makedirs(os.path.join(outdir, dataset), exist_ok=True)
    output = parse_exe_output()
    network_data = get_network_data(dataset)
    file_idx = 1
    for network_var in range(len(network_data['dom_size'])):
        if network_data['leaf'][network_var]:
            dom_size = network_data['dom_size'][network_var]
            for variable_value in range(dom_size):
                n_clause = output['nclauses'] + len(output['variable_map'][network_var][variable_value])
                with open(os.path.join(outdir, dataset, f'{file_idx}.cnf'), 'w') as f:
                    f.write(f'p cnf {output["nvars"]} {n_clause}\n')
                    for v in output['weights']:
                        f.write(f'c p weight {v} {output["weights"][v]} 0\n')
                    f.write(''.join(output["clauses"]))
                    
                    for value_indicator_variables in output['variable_map'][network_var][variable_value]:
                        f.write(f'{value_indicator_variables} 0\n')
                file_idx += 1


def enc3(dataset):
    bash_cmd = ['./bn2cnf_linux', '-i', f'{str(uai_dir)}/{dataset}.uai', '-o', 'tmp.cnf', '-w', 'tmp.weight', '-v', 'tmp.map']
    make_enc_encoding(dataset, bash_cmd, enc3_dir)

def enc4(dataset):
    bash_cmd = ['./bn2cnf_linux', '-i', f'{str(uai_dir)}/{dataset}.uai', '-o', 'tmp.cnf', '-w', 'tmp.weight', '-v', 'tmp.map', '-s', 'prime']
    make_enc_encoding(dataset, bash_cmd, enc4_dir)

def enc4linp(dataset):
    bash_cmd = ['./bn2cnf_linux', '-i', f'{str(uai_dir)}/{dataset}.uai', '-o', 'tmp.cnf', '-w', 'tmp.weight', '-v', 'tmp.map', '-s', 'prime', '-e', 'LOG', '-implicit']
    make_enc_encoding(dataset, bash_cmd, enc4linp_dir)

instances = [f.split('.')[0] for f in os.listdir(uai_dir) if os.path.isfile(os.path.join(uai_dir, f))]

for d in [enc3_dir, enc4_dir, enc4linp_dir]:
    f = os.path.join(d, 'preprocess_time.csv')
    if os.path.isfile(f):
        os.remove(f)

    with open(f, 'w') as f:
        f.write('dataset,time\n')

for instance in instances:
    print(instance)
    enc3(instance)
    enc4(instance)
    enc4linp(instance)

os.remove(os.path.join(script_dir, 'tmp.cnf'))
os.remove(os.path.join(script_dir, 'tmp.weight'))
os.remove(os.path.join(script_dir, 'tmp.map'))
